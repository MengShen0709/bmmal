...
class PretrainedR2Plus1D(nn.Module):
    def __init__(self, weights):
        super(PretrainedR2Plus1D, self).__init__()
        r2p1d = torchvision.models.video.r2plus1d_18(weights=weights)
        self.model = nn.Sequential(
            *list(r2p1d.children())[:-2],
...
def get_pretrained_resnet2p1d(weights=torchvision.models.video.R2Plus1D_18_Weights.DEFAULT):
    return PretrainedR2Plus1D(weights)


def generate_video_resnet2p1d(model_depth, n_input_channels=3, **kwargs):
    assert model_depth in [10, 18, 34, 50, 101, 152, 200]

    if model_depth == 10:
        model = ResNet(BasicBlock, [1, 1, 1, 1], get_inplanes(), n_input_channels=n_input_channels, **kwargs)
    elif model_depth == 18:
        model = ResNet(BasicBlock, [2, 2, 2, 2], get_inplanes(), n_input_channels=n_input_channels, **kwargs)
    elif model_depth == 34:
        model = ResNet(BasicBlock, [3, 4, 6, 3], get_inplanes(), n_input_channels=n_input_channels, **kwargs)
    elif model_depth == 50:
        model = ResNet(Bottleneck, [3, 4, 6, 3], get_inplanes(), n_input_channels=n_input_channels, **kwargs)
    elif model_depth == 101:
        model = ResNet(Bottleneck, [3, 4, 23, 3], get_inplanes(), n_input_channels=n_input_channels, **kwargs)
    elif model_depth == 152:
        model = ResNet(Bottleneck, [3, 8, 36, 3], get_inplanes(), n_input_channels=n_input_channels, **kwargs)
    elif model_depth == 200:
        model = ResNet(Bottleneck, [3, 24, 36, 3], get_inplanes(), n_input_channels=n_input_channels, **kwargs)
