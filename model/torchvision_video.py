...
class VideoResNet(nn.Module):
    ...
    def forward(self, x: Tensor, nl_gate=False) -> Tensor or Tuple[Tensor, Tensor]:
        x = self.stem(x)
...
class VideoResNetLayer4(nn.Module):
    def __init__(
        self,
        block: Type[Union[BasicBlock, Bottleneck]] = BasicBlock,
        conv_makers: Sequence[Type[Union[Conv3DSimple, Conv3DNoTemporal, Conv2Plus1D]]] = [Conv2Plus1D] * 4,
...
def _video_resnet(
    ...
    **kwargs: Any,
    ...
    if pretrain:
        ...
        print(f"load pretrained weights from torchvision")
...
def r2plus1d_18(pretrain=True, **kwargs: Any) -> VideoResNet:
...
if __name__=="__main__":
    model = r2plus1d_18()
    video = torch.rand(1, 3, 15, 112, 112)
    x, z = model(video, nl_gate=True)
    print(x.size())
    layer4 = VideoResNetLayer4()
    print(model)
    print(layer4)
    print(layer4(z).size())
    print(x.size(), z.size())
    target_T = 18
    orig_T = z.size(2)
    z = torch.nn.functional.adaptive_avg_pool2d(z, 1)
    z = torch.flatten(z, start_dim=3)
    z = torch.tile(z, dims=(1, 1, 1, 32))
    z = torch.repeat_interleave(z, repeats=target_T // orig_T, dim=2)
    z = torch.cat([z, z[:, :, -1, :].unsqueeze(2).tile((1,1,target_T % orig_T,1))], dim=2)
    print(z.size())
